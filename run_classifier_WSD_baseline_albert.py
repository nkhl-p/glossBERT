...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, start_id, end_id, related_index_start, related_index_end, text_b=None, label=None):
        ...
        self.end_id = end_id
        self.related_index_start=related_index_start
        self.related_index_end=related_index_end
...
class DataProcessor(object):
    ...
    def get_labels(self, label_data_dir):
...
class WSDProcessor(DataProcessor):
    ...
    def get_labels(self, label_data_dir):
        """See base class."""
        with open(os.path.join(label_data_dir,"sense2index.pkl"),"rb") as f:
            sense2index = pickle.load(f)
        label = []
        for i in sense2index:
            label.append(i)
        return label
    ...
    def _create_examples(self, lines, set_type, lemma2index_dict):
        ...
        for (i, line) in enumerate(lines):
            ...
            lemma = str(line[4])
            label = str(line[-1])
            related_index = lemma2index_dict[lemma]
            related_index_start = related_index[0]
            related_index_end = related_index[-1] + 1
            ...
            if i%1000==0:
                ...
                print("related_index_start", related_index_start)
                print("related_index_end", related_index_end)

            examples.append(
                InputExample(guid=guid, text_a=text_a, start_id=start_id, end_id=end_id, 
                related_index_start=related_index_start, related_index_end=related_index_end, text_b=None, label=label))
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, target_mask, label_mask=None, index_start=None, index_end=None):
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        if example.guid.startswith('dev'):
            # The mask has 0 for related labels and -int for others
            label_mask = [float("-inf")] * len(label_list)
            for i in range(example.related_index_start, example.related_index_end):
                label_mask[i] = 0
            index_start = None
            index_end = None
        else:
            label_mask = None
            index_start = [example.related_index_start]
            index_end = [example.related_index_end]
...
def main():
    ...
    model = AlbertForTokenClassification.from_pretrained('albert-base-v2', config = config)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                result = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=None)
                
                # logits = result.logits + all_label_mask
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        with open(os.path.join(args.output_dir, "results.txt"),"w") as f:
            for input_ids, input_mask, segment_ids, label_ids, target_mask, label_mask in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    result = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=Nones)
                logits = result.logits
                logits = logits[:, -1, :]
                logits = logits.squeeze(-2)
